...
// Production-safe compatibility migration for databases created by older
// Peacely versions. CREATE IF NOT EXISTS does not change an existing table,
// so explicitly add every legacy column used by the current read/write paths.
try {
  await query(`
    ...
    ALTER TABLE maintenance_tickets ADD COLUMN IF NOT EXISTS status VARCHAR(30) DEFAULT 'Open';
  ...
  // Do not prevent the application from booting because of a non-critical
  // migration warning. Individual routers also retry their table setup.
