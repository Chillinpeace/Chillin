...
router.get('/expenses', requireOwner, async (req, res) => {
  try {
    ...
    if (propertyId) {
      const id = Number(propertyId);
      if (!Number.isInteger(id) || id < 1) return res.status(400).json({ success: false, error: 'Invalid property.' });
      params.push(id);
...
router.get('/expenses/summary', requireOwner, async (req, res) => {
  try {
    ...
    const months = await query(`
      SELECT TO_CHAR(DATE_TRUNC('month', e.expense_date), 'YYYY-MM') AS month, COALESCE(SUM(e.amount),0) AS amount, COUNT(*)::INTEGER AS count
      FROM expenses e
      WHERE e.owner_id = $1 AND e.expense_date >= DATE_TRUNC('month', CURRENT_DATE) - INTERVAL '11 months'
