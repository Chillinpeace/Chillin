...
async function buildInvoicePdf(invoice) {
  ...
  const cardX = 38;
  const cardY = 34;
  const cardW = pageWidth - 76;
  const cardH = pageHeight - 68;

  doc.roundedRect(cardX, cardY, cardW, cardH, 18).fill(white);

  const headerH = 238;
  doc.save();
  doc.roundedRect(cardX, cardY, cardW, headerH, 18).clip();
  doc.rect(cardX, cardY, cardW, headerH).fill(navy);
  doc.restore();

  doc.fillColor(white)
    .font('Helvetica-Bold')
    .fontSize(22)
    .text('Peacely', cardX, cardY + 28, { width: cardW, align: 'center' });

  const cx = pageWidth / 2;
  const cy = cardY + 92;

  doc.circle(cx, cy, 31).fill(green);
  doc.circle(cx, cy, 22).fill(navy);
  doc.strokeColor(green).lineWidth(3.2);
  doc.moveTo(cx - 9, cy)
    .lineTo(cx - 2, cy + 7)
    .lineTo(cx + 11, cy - 9)
    .stroke();
  ...
  doc.fillColor(white)
    .font('Helvetica-Bold')
    .fontSize(23)
    .text('Payment Successful', cardX, cardY + 137, {
      width: cardW,
      align: 'center',
    });

  doc.fillColor('#D7E0EC')
    .font('Helvetica')
    .fontSize(10.5)
    .text(
      ...
      cardX + 25,
      cardY + 174,
      { width: cardW - 50, align: 'center' },
    );

  const amountY = cardY + headerH + 28;

  doc.fillColor(muted)
    .font('Helvetica')
    .fontSize(10)
    .text('AMOUNT PAID', cardX + 28, amountY, {
      width: cardW - 56,
      align: 'center',
    });
  ...
  const rows = [
    ...
    ['Tenant', clean(invoice.tenant_name) || '-'],
    ['Property', clean(invoice.property_name) || '-'],
    ['Month', clean(invoice.month) || '-'],
    ['Due date', formatDate(invoice.due_date)],
    ['Payment date', formatDate(invoice.paid_at)],
    ['Payment method', clean(invoice.payment_method) || 'Owner confirmed'],
  ];

  const detailH = rows.length * rowH + 22;

  doc.roundedRect(detailX, detailY, detailW, detailH, 12).fill('#F7F9FC');
  doc.roundedRect(detailX, detailY, detailW, detailH, 12)
    .lineWidth(1)
    .strokeColor(border)
    .stroke();

  rows.forEach(([label, value], index) => {
    const y = detailY + 11 + index * rowH;

    if (index > 0) {
      doc.strokeColor(border).lineWidth(0.6)
        .moveTo(detailX + 14, y - 5)
        .lineTo(detailX + detailW - 14, y - 5)
        .stroke();
    }
    ...
    doc.fillColor(muted)
      .font('Helvetica')
      .fontSize(9.5)
      .text(label, detailX + 16, y + 3, { width: 92 });

    doc.fillColor(text)
      .font('Helvetica-Bold')
      .fontSize(9.5)
      .text(value, detailX + 112, y + 3, {
        width: detailW - 128,
        align: 'right',
        ellipsis: true,
      });
  ...
  const noteY = detailY + detailH + 22;
  ...
  doc.fillColor(muted)
    ...
    .fontSize(9.5)
    .text(
      'This receipt was generated by Peacely for your rent payment.',
      cardX + 28,
      noteY + 20,
      { width: cardW - 56, align: 'center' },
    );
  ...
  doc.fillColor('#9AA5B5')
    .font('Helvetica')
    .fontSize(8.5)
    .text(
      `Invoice ${clean(invoice.invoice_number) || '-'} • Peacely`,
      cardX + 24,
      footerY,
