...
async function ensureSchema() {
  await query(`
    CREATE TABLE IF NOT EXISTS maintenance_tickets (
      ...
      bed_id INTEGER,
      tenant_id INTEGER,
      title VARCHAR(255) NOT NULL DEFAULT 'Maintenance work',
...
router.get('/maintenance', async (req,res) => {
  try {
    const r = await query(`
      SELECT m.id,m.owner_id,m.property_id,m.room_id,m.bed_id,m.tenant_id,m.title,m.description,
             m.category,m.priority,m.status,m.assigned_to,m.estimated_cost,m.actual_cost,m.due_date,
    ...
    res.status(500).json({success:false,error:'Unable to load maintenance records.'});
  }
});
...
router.post('/maintenance', async (req,res) => {
  ...
  const location=await query(`
    ...
    WHERE p.id=$1 AND r.id=$2 AND b.id=$3 AND p.owner_id=$4
    LIMIT 1
  ...
  try {
    ...
    const maintenance=await client.query(`
      INSERT INTO maintenance_tickets(owner_id,property_id,room_id,bed_id,title,description,category,priority,status,actual_cost,due_date,resolved_at,created_at,updated_at)
      VALUES($1,$2,$3,$4,$5,$6,$7,'Medium','Resolved',$8,$9,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)
    ...
    const expense=await client.query(`
      INSERT INTO expenses(owner_id,property_id,expense_date,category,amount,note,created_at,updated_at)
      VALUES($1,$2,$3,'Maintenance',$4,$5,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)
    ...
    return res.status(201).json({
      success:true,
      maintenance:{...maintenance.rows[0],property_name:location.rows[0].property_name,room_number:location.rows[0].room_number,bed_number:location.rows[0].bed_number},
