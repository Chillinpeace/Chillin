...
function Rooms() {
    ...
    if (stored) {
      try {
        ...
        const normalized = parsed.map((room) => ({
          ...room,
          bedStatus:
            room.bedStatus && room.bedStatus.length === room.beds
              ? room.bedStatus
              : Array.from(
                  { length: room.beds },
  ...
  const addRoom = () => {
    if (!number.trim() || !beds || !rent) {
      alert("Please fill Room Number, Number of Beds and Monthly Rent.");
      return;
    }
  ...
  const deleteRoom = (roomId: number) => {
    ...
    if (room.bedStatus?.some((status) => status === "occupied")) {
      alert("This room has occupied beds. Please remove the tenants first.");
      return;
    }
    ...
    saveRooms(rooms.filter((item) => item.id !== roomId));
  };
  ...
  const occupiedBeds = rooms.reduce((sum, room) => {
    ...
    return (
      sum +
      statuses.filter((status) => status === "occupied").length
    );
  }, 0);
...
function Tenants() {
  ...
  useEffect(() => {
    const storedTenants = localStorage.getItem(tenantsStorageKey);
    const storedRooms = localStorage.getItem(roomsStorageKey);
  ...
  const addTenant = () => {
    if (!name.trim() || !phone.trim() || !roomId || bedIndex === "") {
      alert(
  ...
  const removeTenant = (tenant: Tenant) => {
    ...
    const updatedRooms = rooms.map((room) => {
      ...
      if (
        updatedStatuses[tenant.bedIndex] !== undefined
      ) {
        updatedStatuses[tenant.bedIndex] = "available";
      }
    ...
    <div className="tenant-list-page">
            ...
            <div className="form-field">
              ...
              >
                ...
                {availableBeds.map((bed) => (
                  ...
                  >
                    Bed{" "}
                    {String.fromCharCode(65 + bed.index)}
                  </option>
                ))}
              </select>
            </div>
      ...
      {tenants.length === 0 ? (
        <div className="panel empty-state">
          ...
          <p>
            Add your first tenant to start managing admissions.
          </p>
        </div>
      ) : (
        <div className="tenant-list">
          {tenants.map((tenant) => (
            <div className="panel tenant-card" key={tenant.id}>
              ...
              <div className="tenant-details">
                ...
                <div>
                  <span>Deposit</span>
                  <strong>
                    ₹{tenant.deposit.toLocaleString("en-IN")}
                  </strong>
                </div>
              </div>
...
function RentPayments() {
  ...
  useEffect(() => {
    ...
    const storedPayments =
      localStorage.getItem(paymentsStorageKey);
  ...
  const totalPending = Math.max(
    totalMonthlyRent - totalCollected,
    0
  );

  const tenantPaidAmount = (id: number) => {
  ...
  const getTenantStatus = (tenant: Tenant) => {
    const paid = tenantPaidAmount(tenant.id);
    ...
    <div className="rent-page">
      <div className="page-header">
        <div>
          ...
          <h2>Rent & Payments</h2>
          <p className="page-description">
            Track monthly rent and record tenant payments.
          </p>
        </div>
      ...
      <div className="panel">
        ...
        {tenants.length === 0 ? (
          <div className="empty-state">
            ...
            <p>
              Add tenants first, then you can record their rent.
            </p>
          </div>
        ) : (
...
function ComingSoon({ title }: { title: string }) {
  return (
    <div className="coming-soon">
      ...
      <p>
        This module is coming next. We are building Peacely
        step by step.
      </p>
    </div>
  );
